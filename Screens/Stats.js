...
const AccordionItem = memo(({ title, totalProfit, houseBreakdowns }) => {
  ...
  return (
    <View style={[styles.breakdownCard, { overflow: 'hidden', padding: 0 }]}>
      <TouchableOpacity 
        style={[
      ...
      >
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          <Text style={[breakdownStyles.itemProfit, { color: profitColor, marginRight: 10 }]}>
      ...
      {expanded && (
        <View style={{ backgroundColor: "#F1F5F9", paddingTop: 5, paddingBottom: 0 }}> 
          {activeHouses.length > 0 ? activeHouses.map((houseInfo, index) => {
            const hProfitColor = houseInfo.profit >= 0 ? COLORS.positive : COLORS.negative;
            ...
            return (
              ...
              >
                <View style={{ flexDirection: "row", alignItems: "center" }}>
                  ...
                  <Text style={{ fontSize: 14, fontWeight: "600", color: COLORS.textGray }}>
                    {houseInfo.house}
                  </Text>
                </View>
                <Text style={{ fontSize: 14, fontWeight: "bold", color: hProfitColor }}>
                  {hProfitSign}{houseInfo.profit.toFixed(2)}€
                </Text>
              </View>
            );
          }) : (
             <Text style={[breakdownStyles.noData, { paddingVertical: 15 }]}>No house data available.</Text>
          )}
        </View>
      )}
    </View>
...
const BetTypeAnalysisCard = ({ data }) => {
  ...
  return (
    <View style={styles.breakdownCard}>
      ...
      <View style={breakdownStyles.container}>
        {data.map((item, index) => {
          const profitColor = item.total_profit >= 0 ? COLORS.positive : COLORS.negative;
          ...
          return (
            <View key={index} style={breakdownStyles.itemRow}>
              <View style={breakdownStyles.itemLeft}>
                <Ionicons 
                  name={item.bet_type === 'Single Bets' ? 'scan-outline' : 'layers-outline'} 
                  size={18} 
                  color={COLORS.textGray} 
                  style={{ marginRight: 8 }} 
                />
              ...
              <Text style={[breakdownStyles.itemProfit, { color: profitColor }]}>
                {profitSign}{item.total_profit ? item.total_profit.toFixed(2) : "0.00"}€
              </Text>
            </View>
          );
        })}
      ...
      <View style={styles.chartCard}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15 }}>
          ...
          <View style={{ flexDirection: 'row', backgroundColor: COLORS.background, borderRadius: 8, padding: 2 }}>
            <TouchableOpacity onPress={() => setTimeFilter('30d')} style={{ paddingHorizontal: 10, paddingVertical: 4, backgroundColor: timeFilter === '30d' ? COLORS.primary : 'transparent', borderRadius: 6 }}>
              <Text style={{ fontSize: 12, fontWeight: 'bold', color: timeFilter === '30d' ? '#FFF' : COLORS.textGray }}>30D</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => setTimeFilter('12m')} style={{ paddingHorizontal: 10, paddingVertical: 4, backgroundColor: timeFilter === '12m' ? COLORS.primary : 'transparent', borderRadius: 6 }}>
              <Text style={{ fontSize: 12, fontWeight: 'bold', color: timeFilter === '12m' ? '#FFF' : COLORS.textGray }}>12M</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => setTimeFilter('all')} style={{ paddingHorizontal: 10, paddingVertical: 4, backgroundColor: timeFilter === 'all' ? COLORS.primary : 'transparent', borderRadius: 6 }}>
              <Text style={{ fontSize: 12, fontWeight: 'bold', color: timeFilter === 'all' ? '#FFF' : COLORS.textGray }}>ALL</Text>
            </TouchableOpacity>
          </View>
        </View>
...
const Stats = () => {
    ...
    return (
        ...
        <View style={{ paddingHorizontal: 15, paddingBottom: 20 }}>
          <Text style={styles.cardTitle}>
            {isSports ? "Profit by Sport & House" : "Profit by Game & House"}
          </Text>
          
          {singleSportGameData.map((mainItem) => {
            const specificHouseBreakdowns = currentData.combinedBreakdown.filter((combo) => 
              isSports ? combo.sport === mainItem.name : combo.game === mainItem.name
            );
            ...
            return (
              <AccordionItem 
                key={mainItem.id}
                title={mainItem.name}
                totalProfit={mainItem.profit}
                houseBreakdowns={specificHouseBreakdowns}
              />
            );
          })}
        </View>
